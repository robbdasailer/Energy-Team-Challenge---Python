...
for t in time_horizon:
    point_source_availability[t-1] = round(m*(t+1) + b,2)
    if t > 25:
...
objective = - 0.8339730893 * init_investment_var + gp.quicksum(
...
# The CO2 demand of the FT synthesis must not exceed the CO2 availability
for t in time_horizon:
...
for t in time_horizon:
    model.addConstr(point_source_amount[t-1] * x_point_source[t-1] + dac_amount[t-1] * x_dac[t-1] == transported_jetfuel[t-1] / 0.71 * CO2_demand_per_unit_jetfuel)
...
# The capacity of the Fischer Tropsch Synthesis must meet demand of airport
for t in time_horizon:
...
NPV_t = {}
NPV_t[0] = - 1/5 * init_investment_var.x 
NPV_t[1] = NPV_t[0] - 1/5 * init_investment_var.x / (1+i)
NPV_t[2] = NPV_t[1] - 1/5 * init_investment_var.x / (1+i)**2
NPV_t[3] = NPV_t[2] - 1/5 * init_investment_var.x / (1+i)**3
NPV_t[4] = NPV_t[3] - 1/5 * init_investment_var.x / (1+i)**4
# NPV_t[5] = NPV_t[4] - 1/6 * init_investment_var.x / (1+i)**5


for t in time_horizon:
    NPV_t[t+4] = NPV_t[t+3] + ( model.getVarByName(f'y_Customer_1_Steel_Plant_{t}').x * price_per_unit['hydrogen']
    ...
    - cash_outflow_battery.getValue()) / ((1 + i) ** (t+4))
for index in NPV_t:
    print(NPV_t[index])
